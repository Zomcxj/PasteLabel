...
def test_mode_segment_places_annotate_button_before_paste_button():
    source = (ROOT / "PasteY" / "ui" / "ui_builder.py").read_text(encoding="utf-8")
...
def test_stats_button_uses_stats_copy_in_both_languages():
    ui_source = (ROOT / "PasteY" / "ui" / "ui_builder.py").read_text(encoding="utf-8")
    i18n_source = (ROOT / "PasteY" / "ui" / "i18n.py").read_text(encoding="utf-8")
...
def test_image_editor_starts_in_annotate_mode():
    source = (ROOT / "PasteY" / "ui" / "main_window.py").read_text(encoding="utf-8")

    assert "self.edit_mode = 'annotate'" in source


def test_settings_dialog_uses_param_spin_style_for_magnifier_zoom():
    source = (ROOT / "PasteY" / "ui" / "settings_dialog.py").read_text(encoding="utf-8")
...
def test_theme_styles_qdouble_spinbox_like_other_numeric_inputs():
    source = (ROOT / "PasteY" / "ui" / "theme.py").read_text(encoding="utf-8")
