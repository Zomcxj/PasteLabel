...
from .mixins.processing_panel_builder import ProcessingPanelBuilderMixin, SPLIT_KEYS, TRANSFORM_META
from ..widgets.processing import Worker, CollapsibleSection
...
class ProcessingPanel(ProcessingPanelBuilderMixin, QWidget):
    ...
    def _refresh_texts(self):
        ...
        self._recipe_save_btn.setText(tr("保存为配方"))
        self._recipe_apply_btn.setText(tr("应用配方"))
        self._recipe_del_btn.setText(tr("删除配方"))
    ...
    def _load_recipes(self):
        ...
        from ..engine.pipeline_recipe import normalize_recipe
        recipes = config_manager.load_config().get('pipeline_recipes') or []
        self._recipes = [normalize_recipe(r) for r in recipes]
        combo = getattr(self, '_recipe_combo', None)
        if combo is None:
            return
    ...
    def _collect_pipeline_state(self):
        """收集当前界面流水线配置。"""
        ...
        transforms = {}
        for name, (cb, param_spins, _) in getattr(self, '_aug_widgets', {}).items():
            if name.startswith("__"):
                continue
            params = {pname: [smin.value(), smax.value()]
                      for pname, (smin, smax) in param_spins.items()}
            transforms[name] = {'checked': cb.isChecked(), 'params': params}
        return {
            'steps': steps,
            'augment': {
            ...
            'split': {
                'train': self._split_train.value(),
                'val': self._split_val.value(),
                'test': self._split_test.value(),
            },
        }
    ...
    def _save_recipe(self):
        ...
        if not ok or not name:
            return
        existing = {r['name'] for r in getattr(self, '_recipes', [])}
        if name in existing:
            reply = QMessageBox.question(
                self, tr("保存为配方"), tr("覆盖同名配方"),
    ...
    def _apply_recipe(self):
        ...
        for name, (cb, param_spins, _) in getattr(self, '_aug_widgets', {}).items():
            if name.startswith("__"):
                continue
            entry = aug['transforms'].get(name)
            if entry is None:
                continue
            cb.setChecked(entry['checked'])
            for pname, (smin, smax) in param_spins.items():
                pair = entry['params'].get(pname)
                if pair:
                    smin.setValue(int(round(pair[0])) if isinstance(smin, QSpinBox) else pair[0])
                    smax.setValue(int(round(pair[1])) if isinstance(smax, QSpinBox) else pair[1])
        self._aug_ratio.setValue(aug['ratio'])
        self._aug_mode.setCurrentIndex(0 if aug['mode'] == 'all' else 1)
        self._aug_inc_orig.setChecked(aug['include_original'])
        self._aug_skip_empty.setChecked(aug['skip_empty'])
        fmt_idx = self._exp_format.findText(r['export']['format'])
        if fmt_idx >= 0:
            self._exp_format.setCurrentIndex(fmt_idx)
        self._exp_skip_empty.setChecked(r['export']['skip_empty'])
        available = list(getattr(self, '_exp_label_checkboxes', {}).keys())
        chosen = apply_recipe_to_labels(r, available)
        for lbl, cb in self._exp_label_checkboxes.items():
            cb.setChecked(lbl in chosen)
