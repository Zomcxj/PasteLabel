...
#    --use-spec    : 使用 PasteLabel_optimized.spec 文件构建（推荐）
#
#  快捷方式:
#    ./build.sh           # 完整构建（发布版本）
#    ./build.sh fast      # 快速构建（日常开发）= --use-spec --no-clean
# ==============================================================================
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            echo "未知参数: $1"
            echo "用法: $0 [--clean|--no-clean] [--use-spec] | fast"
            echo ""
            echo "示例:"
            echo "  $0           # 完整构建（发布版本）"
            echo "  $0 fast      # 快速构建（日常开发）"
            echo "  $0 --use-spec --no-clean  # 等同于 fast"
            exit 1
            ;;
    esac
done
...
if [ "$PLATFORM" = "windows" ]; then
    ...
    OUTPUT_FILE="dist/PasteLabel.exe"
else
    PYINSTALLER_CMD="pyinstaller"
    ADD_DATA_SEP=":"
    EXTRA_ARGS=""
    OUTPUT_FILE="dist/PasteLabel"
fi
...
# 检查 spec 文件（如果使用）
SPEC_FILE="PasteLabel_optimized.spec"
if [ "$USE_SPEC" = true ]; then
    if [ ! -f "$SPEC_FILE" ]; then
        log_error "Spec 文件 '$SPEC_FILE' 不存在"
        exit 1
    fi
    log_ok "使用 Spec 文件构建: $SPEC_FILE"
fi
...
if [ "$USE_SPEC" = true ]; then
    ...
    [ "$CLEAN_BUILD" = true ] && PYINSTALLER_SPEC_CMD="$PYINSTALLER_SPEC_CMD --clean"
    ...
    eval "$PYINSTALLER_SPEC_CMD --noconfirm \"$SPEC_FILE\""
else
    # 使用命令行参数构建
    log_info "执行命令:"
    echo "─────────────────────────────────────────"
