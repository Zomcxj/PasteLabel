...
from PyQt5.QtWidgets import (
    ...
    QMenu, QInputDialog, QAction, QDialog, QProgressDialog
)
from PyQt5.QtGui import QPixmap, QPainter, QColor, QBrush, QPen, QIcon, QImage
from PyQt5.QtCore import Qt, QPoint, QPointF, QRectF, QSizeF, QSize, QThread, pyqtSignal, QEvent, QTimer
import os
import re
...
class ImageEditor(QMainWindow):
    def __init__(self):
        ...
        # 贴图列表显示模式
        self.is_thumbnail_mode = True  # 默认为缩略图模式

        # 缩略图网格配置（可调整参数） 列表宽度300像素
        self.thumbnail_grid_width = 56  # 网格宽度（像素）
        self.thumbnail_grid_height = 60  # 网格高度（像素）
        # self.thumbnail_cols = 5  # 每行显示的列数
        self.thumbnail_spacing = 2  # 网格间距（像素）
    ...
    def init_ui(self):
        ...
        # 依次在脚本目录、父目录（项目根目录）中查找图标
        def _find_icon(name):
            for base in [script_dir, os.path.dirname(script_dir)]:
                p = os.path.join(base, "ico_image", name)
                if os.path.exists(p):
                    return p
            return os.path.join(script_dir, "ico_image", name)
        file_icon_path = _find_icon("file-os.png")
        folder_icon_path = _find_icon("folders.png")
        ...
        self.upload_a_btn.setMaximumWidth(30)
        self.upload_a_btn.setStyleSheet("""
            QPushButton {
        ...
        self.load_folder_btn.setMaximumWidth(30)
        ...
        self.upload_b_btn.setMaximumWidth(30)
        self.upload_b_btn.setStyleSheet("""
            QPushButton {
                background-color: transparent;
                border: 1px solid transparent;
                border-radius: 4px;
        ...
        self.load_small_folder_btn.setMaximumWidth(30)
        self.load_small_folder_btn.setStyleSheet("""
            QPushButton {
                background-color: transparent;
                border: 1px solid transparent;
                border-radius: 4px;
        ...
        # 添加贴图标签上传按钮
        upload_layout.addSpacing(2)
        self.upload_paste_label_btn = QPushButton("")
        if os.path.exists(file_icon_path):
            self.upload_paste_label_btn.setIcon(QIcon(file_icon_path))
        self.upload_paste_label_btn.clicked.connect(self.upload_paste_labels)
        self.upload_paste_label_btn.setMaximumWidth(30)
        self.upload_paste_label_btn.setStyleSheet("""
            QPushButton {
                background-color: transparent;
                border: 1px solid transparent;
                border-radius: 4px;
        ...
        """)
        upload_layout.addWidget(self.auto_save_checkbox)
        ...
        self.show_label_names_checkbox.setStyleSheet("""
            QCheckBox {
                spacing: 8px;
                font-size: 12px;
            }
        ...
        self.auto_label_checkbox.setStyleSheet("""
            QCheckBox {
                spacing: 8px;
                font-size: 12px;
            }
        ...
        """)
        upload_layout.addWidget(self.auto_label_checkbox)
        
        # 添加图片名前缀勾选框
        upload_layout.addSpacing(10)
        self.prefix_checkbox = QCheckBox("添加文件名前缀")
        self.prefix_checkbox.setChecked(True)
        self.prefix_checkbox.setStyleSheet("""
            QCheckBox {
                spacing: 8px;
                font-size: 12px;
            }
        ...
        """)
        upload_layout.addWidget(self.prefix_checkbox)
        ...
        self.paste_label_list.setStyleSheet("""
            QListWidget {
                border: 1px solid #E0E0E0;
                border-radius: 8px;
                background-color: white;
        ...
        self.random_paste_btn.setStyleSheet("""
            QPushButton {
                background-color: #E3F2FD;
        ...
        self.batch_paste_btn.setStyleSheet("""
            QPushButton {
                background-color: #E3F2FD;
                border: 1px solid #2196F3;
                border-radius: 12px;
        ...
        # 初始设置范围
        self.on_min_size_changed(self.min_size_spin.value())
        self.on_max_size_changed(self.max_size_spin.value())
        
        paste_params_layout.addStretch()
        control_layout.addLayout(paste_params_layout)
        ...
        self.small_list.setIconSize(QSize(self.thumbnail_grid_width, self.thumbnail_grid_height))
        self.small_list.setGridSize(QSize(self.thumbnail_grid_width, self.thumbnail_grid_height + 20))  # 增加高度以容纳文件名
    ...
    def upload_small_images(self):
        ...
        if hasattr(self, 'is_thumbnail_mode'):
            if self.is_thumbnail_mode:
                self.small_list.setViewMode(QListWidget.IconMode)
                self.small_list.setIconSize(QSize(self.thumbnail_grid_width, self.thumbnail_grid_height))
                self.small_list.setGridSize(QSize(self.thumbnail_grid_width, self.thumbnail_grid_height + 20))
                self.small_list.setSpacing(self.thumbnail_spacing)
                self.small_list.setWrapping(True)
                self.small_list.setFlow(QListWidget.LeftToRight)
                self.small_list.setResizeMode(QListWidget.Adjust)
    ...
    def add_list_item(self, file_name, pixmap):
        """添加列表项，根据当前模式显示缩略图或文件名"""
        item = QListWidgetItem(file_name)
        
        if self.is_thumbnail_mode:
            # 缩略图模式：显示缩略图和文件名
            thumb_pixmap = self.create_thumbnail(pixmap, self.thumbnail_grid_width, self.thumbnail_grid_height)
    ...
    def create_thumbnail(self, pixmap, max_width, max_height):
        """
        创建缩略图，等比缩放到网格大小，边缘填充透明背景
        :param pixmap: 原始图片
        :param max_width: 网格宽度
        :param max_height: 网格高度
        :return: 缩略图QPixmap（固定大小）
        """
    ...
    def toggle_view_mode(self):
        ...
        self.small_list.updateGeometry()
        self.small_list.repaint()
        # self.small_list.verticalScrollBar().setValue(0)
    ...
    def upload_paste_labels(self):
        """上传贴图标签txt文件，读取txt空格隔开选第一部分，更新贴图标签"""
        ...
        if file_path:
            try:
                ...
                if labels:
                    ...
                    # QMessageBox.information(self, "成功", f"已加载 {len(labels)} 个贴图标签")
                else:
                ...
                if label_part:
                    ...
                    if hasattr(self, 'paste_label_list'):
                        ...
                        for i in range(self.paste_label_list.count()):
                            ...
                            # 提取纯标签名称，去除"(count)"部分
                            if " (" in existing_label:
                                pure_label = existing_label.split(" (")[0]
                                if pure_label == paste_label:
                                    label_exists = True
                                    break
                            elif existing_label == paste_label:
                                label_exists = True
                                break
                        # 如果标签不存在，添加到列表中
                        if not label_exists:
    ...
    def random_paste_images(self, background=None, detection_boxes=None):
        ...
        for idx in selected_indices:
            ...
            # 随机位置（在背景图范围内） 确保x在50~width-50 y不低于200 
            max_x = min(max(50, current_background.width() - new_width), current_background.width() - 100)
            ...
            for _ in range(max_attempts):
                ...
                for box in boxes:
                    ...
                    if x2 <= x1 or y2 <= y1:
                        ...
                        iou = intersection / area1 if area1 > 0 else 0.0
            ...
            if valid_position:
                ...
                if hasattr(self, 'auto_label_checkbox') and self.auto_label_checkbox.isChecked():
                    # 获取贴图文件名
                    image_path = self.small_images[idx][0]
                    image_name = os.path.basename(image_path)
                    # 按照下划线_拆分，取第一部分作为标签
                    label_part = image_name.split('_')[0]
                    # 移除文件扩展名
    ...
    def get_save_info(self):
        ...
        if self.current_background_index < 0 or self.current_background_index >= len(self.background_images):
            return None
        
        original_file_path = self.background_images[self.current_background_index]
        original_file_name = os.path.basename(original_file_path)
        
        # 生成保存目录：背景图所在目录 + 文件名_paste_output
    ...
    def save_canvas(self):
        ...
        save_info = self.get_save_info()
        if not save_info:
            QMessageBox.warning(self, "警告", "无法获取保存信息")
            return
        ...
        # 双重验证：保存函数返回值 + 文件实际存在性
        formatted_file_path = PathUtils.to_display_path(file_path)
        if save_success and os.path.exists(file_path):
        ...
        # 创建提示标签
        save_label = QLabel(tip_text, self)
        ...
        # 2. 获取背景图片的尺寸
        img_width = self.current_background.width()
        img_height = self.current_background.height()
        # 3. 计算图片在画布中的左上角坐标（居中显示的图片）
        ...
        # 提示显示时间：成功1秒，失败3秒
        show_time = 1000 if save_success and os.path.exists(file_path) else 3000
    ...
    def save_all_canvas(self):
        """保存所有背景图片及其贴图"""
        if not self.background_images:
            QMessageBox.warning(self, "警告", "没有背景图片可保存")
            return
        
        # 创建进度条对话框
        progress_dialog = QProgressDialog("正在保存所有图片...", "取消", 0, len(self.background_images), self)
        ...
        # 完成进度
        progress_dialog.setValue(len(self.background_images))
        ...
        # 显示保存完成消息
        # if saved_count > 0:
        #     QMessageBox.information(self, "完成", f"已保存 {saved_count} 张图片")
    
    def create_app_icon(self):
        """从本地img.png文件加载应用图标"""
        # 获取脚本所在目录
        script_dir = os.path.dirname(os.path.abspath(__file__))
        # 图标文件路径（依次在脚本目录、父目录中查找）
        icon_path = os.path.join(script_dir, "ico_image", "icoo.png")
        if not os.path.exists(icon_path):
            icon_path = os.path.join(os.path.dirname(script_dir), "ico_image", "icoo.png")
...
class Canvas(QWidget):
    ...
    def paintEvent(self, event):
        ...
        for i, (pixmap, rect, label) in enumerate(self.parent.canvas_items):
            if background_rect is not None:
                ...
                is_hovered = item_rect.contains(mouse_pos)
                ...
                if is_selected:
                    ...
                    # 添加边框颜色的透明蒙版（编辑状态加强）
                    overlay_color = QColor(135, 206, 250, 80)  # 天蓝色，80/255透明度
                ...
                if is_selected:
                    # 编辑状态：加强边框颜色
                    pen = QPen(QColor(135, 206, 250), 3)  # 天蓝色边框，更明显
                    painter.setPen(pen)
                    painter.drawRect(
                        int(item_x), int(item_y),
                        int(item_width), int(item_height)
                    ...
                    # 非编辑状态：显示天蓝色边框
                    pen = QPen(QColor(135, 206, 250), 1)  # 天蓝色边框
                    painter.setPen(pen)
                    painter.drawRect(
                        int(item_x), int(item_y),
                        int(item_width), int(item_height)
            ...
            if background_rect is not None:
                for i, box in enumerate(self.parent.detection_boxes):
                    ...
                    br_handle_rect = QRectF(
                        br_handle_center.x() - handle_size,
                        br_handle_center.y() - handle_size,
                        handle_size, handle_size
                    )
                    # 左上角调整手柄
                    ...
                    tr_handle_rect = QRectF(
                        tr_handle_center.x() - handle_size,
                        tr_handle_center.y(),
                        handle_size, handle_size
                    )
                    # 左下角调整手柄
                    ...
                    bl_handle_rect = QRectF(
                        bl_handle_center.x(),
                        bl_handle_center.y() - handle_size,
                        handle_size, handle_size
                    )
    ...
    def wheelEvent(self, event):
        """处理鼠标滚轮事件，实现背景图缩放或选中贴图缩放"""
        if self.parent.current_background is None:
            return
        
        # 如果有贴图被选中，只缩放该贴图
        if self.parent.selected_item is not None and 0 <= self.parent.selected_item < len(self.parent.canvas_items):
            # 获取滚轮角度
            ...
            if self.parent.current_background:
                bg_width = self.parent.current_background.width()
                bg_height = self.parent.current_background.height()
                bg_short_side = min(bg_width, bg_height)
            else:
                bg_short_side = 500  # 默认值
            
            # 限制贴图大小
            min_size = 10
            max_size = bg_short_side * 0.9  # 留10%的边距
        ...
        # 如果背景图标签处于编辑状态，缩放该标签框
            ...
            if self.parent.current_background:
                ...
                bg_width, bg_height = 800, 600  # 默认值
            
            # 限制检测框大小
            min_size = 10
            max_size = min(bg_width, bg_height) * 0.9  # 留10%的边距
            ...
            new_x = max(0, new_x)
            new_y = max(0, new_y)
            new_x = min(new_x, bg_width - new_width)
            new_y = min(new_y, bg_height - new_height)
