# PasteLabel

PasteLabel 是一个基于 PyQt5 的图像标注工具，支持**贴图**（合成）和**检测框**（LabelMe 格式）标注，有效解决样本采集困难的问题。

![界面展示](./ico_image/demo.png)

---

## 支持平台

**Windows 10+** · **Linux (x86_64)**

> 使用 `PasteY/build.sh` 在对应平台自行构建。

📖 详细使用教程请参阅 [用户指南](./user_guide.md)

## 功能特性

- 🎨 **深色/浅色主题** - 一键切换深色和浅色主题
- 🌐 **中英文切换** - 支持中文和英文界面，弹窗按钮同步本地化
- ⚙️ **快捷键自定义** - 可在设置中修改所有快捷键
- 🧤 **巧手记录** - 自动记录最近 10 组背景图、贴图、标签路径，支持备注、删除和按当前图片恢复
- 🖼️ **贴图标注** - 支持随机/批量放置贴图
- 📦 **检测框标注** - LabelMe 格式 JSON 输出
- 🎯 **智能放置** - 中心点避让算法避免贴图重叠
- 📁 **批量处理** - 一键处理所有图片
- 📂 **拖拽操作** - 拖入图片/JSON 上传，拖出文件复制
- 🗂️ **工作/移除路径** - 支持移除图片、恢复到工作路径，并可设置切图步长

## 源码运行

### 依赖

```bash
pip install PyQt5 opencv-python pyinstaller
```

Windows 深色标题栏依赖 `ffi.dll`。使用 `conda` 环境时，该文件通常在：

```text
<conda_env>/Library/bin/ffi.dll
```

`PasteY/build.sh` 会自动从当前 Python 环境查找并打包 `ffi.dll`；如果启动 exe 时提示缺少 `ffi.dll`，请确认打包使用的是正确环境，或将该 DLL 放在 exe 同目录。

### 运行

```bash
python PasteY/main.py
```

## 项目结构

```
PasteLabel/
├── PasteY/                    # 主代码包
│   ├── main.py                # 程序入口
│   ├── __init__.py            # 包初始化
│   ├── build.sh               # Windows/Linux 构建脚本
│   ├── PasteLabel.spec        # Windows PyInstaller 配置
│   ├── canvas/                # 画布模块
│   │   ├── canvas.py          # 画布控件
│   │   ├── canvas_renderer.py # 画布绘制
│   │   ├── canvas_interaction.py # 画布交互
│   │   ├── canvas_drawing.py  # 检测框绘制
│   │   └── canvas_menu.py     # 右键菜单
│   ├── ui/                    # 界面模块
│   │   ├── __init__.py
│   │   ├── main_window.py     # 主窗口逻辑
│   │   ├── ui_builder.py      # UI 构建
│   │   ├── dialogs.py         # 对话框
│   │   ├── dialog_helpers.py  # 统一弹窗主题/居中/按钮文字
│   │   ├── handy_dialog.py    # 巧手记录弹窗
│   │   ├── settings_dialog.py # 设置对话框
│   │   ├── theme.py           # 深色/浅色主题
│   │   ├── i18n.py            # 中英文国际化
│   │   └── dwm.py             # Windows 标题栏颜色
│   ├── engine/                # 业务逻辑
│   │   ├── __init__.py
│   │   ├── paste_engine.py    # 贴图引擎
│   │   ├── image_loader.py    # 图片加载
│   │   ├── label_manager.py   # 标签管理
│   │   ├── save_manager.py    # 保存管理
│   │   ├── undo_manager.py    # 撤销/重做
│   │   └── event_handler.py   # 事件处理
│   ├── core/                  # 核心配置
│   │   ├── __init__.py
│   │   ├── config.py          # 配置常量
│   │   ├── config_manager.py  # 配置管理器
│   │   ├── utils.py           # 工具函数
│   │   ├── editor_protocol.py # 编辑器接口
│   │   └── exception_hook.py  # 异常捕获
├── tests/                     # 单元测试
│   ├── test_config.py
│   ├── test_config_manager.py
│   ├── test_i18n.py
│   └── test_utils.py
├── ico_image/                 # 图标资源
│   ├── demo.png               # 界面展示图
│   ├── icoo.png               # 应用图标
│   └── fonts/                 # JetBrains Mono 字体
├── conftest.py                # pytest 配置
├── paste_label.txt            # 贴图标签文件
├── user_guide.md              # 用户指南
├── README.md                  # 项目说明
├── LICENSE                    # MIT 许可证
└── .gitignore
```

## 测试

```bash
pip install pytest pytest-qt
pytest tests/ -v
```

## 许可证

遵循MIT

Copyright © 2026 Zomcxj
